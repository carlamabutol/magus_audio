...
const Playlist = ({navigation}) =>{
  ...
  const fetchFeatured = async () => {
    const resp = await fetch(global.link+"api/v1/track/"+global.playlist.featured_id);
  ...
  return(
    <>
      ...
      <SafeAreaView style={{...Platform.select({android: {marginTop: -heightScale()}, ios: {marginTop: -heightScale()}}), height: height, width: width, transform: [{scaleX: 1}]}}>
        ...
        <Image preview={{uri: global.playlist.cover}} uri={global.playlist.cover} style={{width: moderateScale(200), height: moderateScale(200), borderRadius: moderateScale(15), alignSelf: 'center', marginBottom: moderateScale(10)}}/>
        <ScrollView showsVerticalScrollIndicator={false} style={{width: width-moderateScale(30), marginHorizontal: moderateScale(15), height: height}}>
          <View>
            <Text style={{fontSize: moderateScale(22), textAlign: 'center', fontWeight: '700'}}>{global.playlist.title}</Text>
            <Text style={{fontSize: moderateScale(14), lineHeight: moderateScale(22), textAlign: 'center', fontWeight: '400'}}>{global.playlist.description}</Text>
          </View>
