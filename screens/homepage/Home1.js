...
import { Dimensions, Modal, ScrollView, ImageBackground, StyleSheet, Image, TouchableOpacity, Text, View, LogBox } from 'react-native';
  ...
  const fetchMusic = async (current) => {
    ...
    global.description=data[0].description
     if( global.length==2){
      ...
      const status = await soundObject.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track1+"&version=1"}, {shouldPlay: true, volume: global.volume1});
      const status1 = await soundObject1.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track2+"&version=1"}, {shouldPlay: true, volume: global.volume2});
      const status2 = await soundObject2.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track3+"&version=1"}, {shouldPlay: true, volume: global.volume3});
      ...
      const status1 = await soundObject1.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track2+"&version=1"}, {shouldPlay: true, volume: global.volume2});
      const status2 = await soundObject2.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track3+"&version=1"}, {shouldPlay: true, volume: global.volume3});
      const status3 = await soundObject3.loadAsync({'uri': "https://dev.node.magusaudio.com/api/v1/s3/audio/track?id="+global.track4+"&version=1"}, {shouldPlay: true, volume: global.volume4});
  ...
  return(
      ...
      <ImageBackground source={require('../../assets/home2bg.png')} style={{width: Dimensions.get('window').width, height: Dimensions.get('window').height, flex: 1}}>
        ...
        <UserContext.Provider value={{value, setValue}}>
          <Tab.Navigator screenOptions={{tabBarShowLabel: false, tabBarStyle: [{height: 80,paddingTop: -10, shadowColor: 'gray', shadowOffset: {width:1, height: 1}, shadowOpacity: 10, paddingBottom: 20, backgroundColor: 'white', borderColor: 'gray', borderTopLeftRadius:25, borderTopRightRadius:25, borderWidth: 0.2, position: 'absolute', paddingLeft:20, paddingRight: 20, elevation: 20,}]}}>
            ...
            <Tab.Screen name="Search" component={PlayStack4}
              options={{headerShown: false ,tabBarIcon: ({focused})=>(
              <View style={{alignItems: 'center', justifyContent: 'center', top: 10}}>
                <Image source={require('../../assets/nav/Search.png')} style={{width: 35, height: 30, tintColor: focused ? 'rgba(24,119,242,1)' : 'black'}}/>
            ...
            <ImageBackground imageStyle={{borderRadius: 10}} style={{backgroundColor: 'rgba(67,156,212,0.9)', justifyContent: 'center', alignItems: 'center', width: Dimensions.get('window').width-20, height: 70, borderRadius: 10, shadowOpacity: 0.2, opacity: 1, marginHorizontal: 10,}}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                ...
                <View style={{width: Dimensions.get('window').width-110, alignItems: 'center', height: 70, flexDirection: 'row' }}>
                  ...
                  <TouchableOpacity onPress={()=> [global.value="OUT", setValue("OUT")]} style={{ marginTop: -70, backgroundColor: 'rgba(4,157,217,0.8)', width: 25, height: 25, alignItems: 'center', justifyContent: 'center', borderRadius: 20}}>
                    <Image source={{uri: 'https://cdn-icons-png.flaticon.com/512/1828/1828945.png'}} style={{width:20, height:20, tintColor: 'white',}}/>
                  </TouchableOpacity>
