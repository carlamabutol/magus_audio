...
const TodayPlaylist =({navigation})=>{
  ...
  const addToNewPLaylist = async (item, text) =>{
    ...
    await fetch(`https://dev.magusaudio.com/api/v1/own/playlist-info/add`, {
      ...
      body: JSON.stringify({
        playlist_id: '', moods_id: '', cover: item.cover,
        user_id: global.id, featured_id: global.subs, title: text
      })
      })
      .then(res =>{
        return res.json();
  ...
  return(
    <ImageBackground source={require('../../assets/homebg.png')} style={{width: width, height: height}}>
      ...
      <View style={{ width: width/1.3, alignSelf: 'center', }}>
        <TextInput placeholder="Enter Playlist Name"  autoCorrect={true} value={text} onChangeText={newText => setText(newText)}  style={{width: width/1.3, height: 35,   fontSize: 14, fontWeight: 'bold', textAlign: 'center', marginTop: 3, }}/>
