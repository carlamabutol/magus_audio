...
import { Dimensions, StatusBar, BackHandler, TextInput, Modal, ScrollView, ImageBackground, StyleSheet, Image, TouchableOpacity, Text, View, LogBox, SafeAreaView } from 'react-native';
...
import Privacy from './me/Helps/Privacy';
import Terms from './me/Helps/Terms';
import SubGuide from './me/Helps/SubGuide';
...
const PlayStack=()=>{
  return(
    <Stack.Navigator initialRouteName='MySubs' screenOptions={{header: ()=>{
      return(
        <>
        <ImageBackground source={require('../assets/Today/todaybg.png')} style={{ flex: 1, alignItems: 'center', justifyContent: 'center', width: '100%'}}>
          
          <View style={{height: moderateScale(100), flexDirection: 'row', width: width-moderateScale(10), alignSelf: 'center', alignItems: 'center', marginTop: height/2.5, justifyContent: 'space-between',}}>
            <Image source={require('../assets/Choose/up1.png')} style={{height: moderateScale(90), marginRight: moderateScale(10), width: moderateScale(90)}}/>
            <Image source={require('../assets/Choose/up3.png')} style={{height: moderateScale(100), marginBottom: -moderateScale(30), width: moderateScale(60)}}/>
          </View>
        </ImageBackground>
...
const PlayStack2=()=>{
  return(
    <Stack.Navigator initialRouteName='Today1' screenOptions={{header: ()=>{
      return(
        <>
        <ImageBackground source={require('../assets/Today/todaybg.png')} style={{ flex: 1, alignItems: 'center', justifyContent: 'center', width: '100%'}}>
          <View style={{height: moderateScale(300), width: width/1.4, alignSelf: 'flex-end', alignItems: 'center', marginTop: height/3, justifyContent: 'center',}}>
            <Image source={require('../assets/Today/todayup2.png')} style={{height: moderateScale(180), width: moderateScale(280)}}/>
          </View>
        </ImageBackground>
...
const PlayStack3=()=>{
  return(
    <Stack.Navigator initialRouteName='Me1' screenOptions={{header: ()=>{
      return(
        <>
        <ImageBackground source={require('../assets/Me/me2/mebg.png')} style={{ flex: 1, alignItems: 'center', justifyContent: 'center', width: '100%'}}>
          <View style={{height: moderateScale(100), flexDirection: 'row', width: width, alignSelf: 'flex-end', alignItems: 'center', marginTop: height/2.6, justifyContent: 'space-between',}}>
            <Image source={require('../assets/Me/me2/up2.png')} style={{marginBottom: moderateScale(30), height: moderateScale(80), width: moderateScale(80)}}/>
            <Image source={require('../assets/Me/me2/up1.png')} style={{height: moderateScale(120), width: moderateScale(100)}}/>
          </View>
        </ImageBackground>
...
const Home =({navigation}) =>{
  ...
  const [modal8, setModal8] = useState(true);
  ...
  const fetchSubs=async()=>{
   ...
   useEffect(() => {
    ...
    return()=> [setUnmount(false), BackHandler.removeEventListener("hardwareBackPress", backActionHandler)]
  ...
  const fetchMusic = async (current) => {
    ...
    setIsPlaying('playing')
     ...
     if( global.length==1){
      const status = await soundObject.loadAsync({'uri': global.node+"api/v1/s3/audio/track?id="+global.track1+"&version=1"}, {shouldPlay: true}, {downloadFirst: true});
  ...
  const fetchPlaylist= async()=>{
    ...
    const data = await resp.json();
    setMyPlaylist(data.reverse())
    global.userPlaylist=data
  }
  const imagess = (item1) =>{
    if(item1.subliminal_count==0){
      return(
        <>
          <CachedImage source={{uri: item1.cover}} cacheKey={item1.playlist_id} style={styles.image}/>
        </>
      )
    }else if(global.data){
      ...
      if(index==-1){
        ...
        return(
          <>
            <CachedImage source={{uri: global.data[index].cover}} cacheKey={global.data[index].cover_name} style={styles.image}/>
          </>
        )
      }      
    }
  }
  const addToNewPLaylist = async (test) =>{
    if(test!=""){
      ...
      body: JSON.stringify({
        moods_id: "", featured_id: global.subs_id,
        user_id: global.id, title: test
      })
      })
      .then(res =>{
        return res.json();
      ...
      .then(async (result) =>{
        if(result.message=="success"){
          ...
          setTest('')
          setColors('#6A98CA')
          setError('Subliminal successfully added to playlist!')
  ...
  const addToOldPlaylist = async (item)=>{
    ...
    body: JSON.stringify({featured_id: global.subs_id, playlist_id: item.playlist_id,
      user_id: global.id,
      })
    })
      .then(res =>{
  ...
  const bottom = () => {
    if(vol==false){
      return(
        <>
          <View style={{}}>
            <Text style={{fontSize: moderateScale(16), fontWeight: '500', textAlign: 'center', color: 'white', marginTop: moderateScale(12),}}>{convertTime(position)}</Text>
            ...
            <View style={{}}>
              <View style={{paddingLeft: moderateScale(10), borderRadius: moderateScale(15), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: moderateScale(80)}}>
                <View style={{width: width-moderateScale(60)}}>
                  ...
                  <Text numberOfLines={1} style={{fontSize: moderateScale(14), color: 'white', fontWeight: '500'}}>{global.category}</Text>
                </View>
              </View>
            </View>
          </View>
          <View style={{marginTop: moderateScale(5), borderRadius: moderateScale(15) }}>
            ...
            <ImageBackground style={{width: width, flex: 1, backgroundColor: 'white', height: height}}>
              ...
              <View style={{height: Dimensions.get('window').height, alignItems: 'center', justifyContent: 'center',  width: Dimensions.get('window').width, flex: 1, marginTop: -height}}>
                ...
                <View style={{width: width, justifyContent: 'space-between', flexDirection: 'row'}}>
                  <TouchableOpacity onPress={()=> [global.modalVisible=(false), setVol(false), setValue('MINIMIZE'), global.value="MINIMIZE", global.count=2]} style={{width: moderateScale(38), height: moderateScale(38), marginTop: moderateScale(10), marginLeft: moderateScale(10)}} >
                  ...
                  <View style={{flexDirection: 'row', marginRight: moderateScale(10)}}>
                    <TouchableOpacity onPress={()=> toggleBottomNavigationView1()} style={{width: moderateScale(38), borderRadius: moderateScale(20), justifyContent: 'center', alignItems: 'center', backgroundColor: '#427AB3', height: moderateScale(38), marginTop: moderateScale(10), marginRight: moderateScale(10)}} >
                ...
                <View style={{height: height/2.8, width: width, justifyContent: 'center', alignItems: 'center'}}>
                    {renderPlayPauseBtn()}
                </View>
                ...
                <View style={styles.bottomView}>
                  <TouchableOpacity onPress={()=> toggleBottomNavigationView()} style={{alignItems: 'center', marginTop: moderateScale(-10), marginBottom: moderateScale(5)}}>
                    <Image source={{uri: 'https://cdn-icons-png.flaticon.com/512/7268/7268569.png'}} style={{width: moderateScale(24), tintColor: '#427AB3', height: moderateScale(24)}} />
                  </TouchableOpacity> 
                  <TouchableOpacity onPress={()=> toggleBottomNavigationView()} style={{backgroundColor: '#427AB3', width: moderateScale(150), alignSelf: 'center', borderRadius: moderateScale(15) }}>
                    <Text style={{fontSize: moderateScale(16), fontWeight: '700', color: 'white', textAlign: 'center', borderRadius: moderateScale(10), padding: moderateScale(7)}}>Guide</Text>
                  </TouchableOpacity>
                </View>
                ...
                >
                  <View style={styles.bottomNavigationView}>
                    <TouchableOpacity onPress={()=> toggleBottomNavigationView()} style={{alignItems: 'center', marginTop: moderateScale(5)}}>
                      <Image source={{uri: 'https://cdn-icons-png.flaticon.com/512/32/32195.png'}} style={{width: moderateScale(24), tintColor: '#427AB3', height: moderateScale(24)}} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={()=> toggleBottomNavigationView()} style={{backgroundColor: '#427AB3', width: moderateScale(150), alignSelf: 'center', borderRadius: moderateScale(15) }}>
                      <Text style={{fontSize: moderateScale(16), fontWeight: '700', color: 'white', textAlign: 'center', borderRadius: moderateScale(10), padding: moderateScale(7)}}>Guide</Text>
                    </TouchableOpacity>
                ...
                >
                  <View style={styles.bottomView1}>
                    <TouchableOpacity onPress={()=> toggleBottomNavigationView1()} style={{alignSelf: 'flex-end', marginTop: moderateScale(10), marginRight: moderateScale(10), borderRadius: moderateScale(59), marginTop: moderateScale(5), backgroundColor: '#427AB3'}}>
                      <Image source={require('../assets/Player/exit.png')} style={{width: moderateScale(17), margin: moderateScale(10), tintColor: 'white', height: moderateScale(17)}} />
                    </TouchableOpacity>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} style={{marginTop: moderateScale(10), height: height-moderateScale(140)}}>
                      <View style={{alignSelf: 'center', alignItems: 'center'}}>
                        <View style={{ width: width/1.3, alignSelf: 'center', marginTop: moderateScale(30)}}>
                          <TextInput placeholder="Enter Playlist Name" value={test} onChangeText={newTet => setTest(newTet)}  style={{width: width/1.3, height: moderateScale(30), color: '#427AB3', fontSize: moderateScale(13), fontWeight: 'bold', textAlign: 'center', marginTop: moderateScale(3), }}/>
                        ...
                        <View style={{justifyContent: 'center', width: width, alignItems: 'center', marginTop: moderateScale(10)}} >
                          <TouchableOpacity onPress={()=> addToNewPLaylist(test)} style={{width: width/1.3, borderColor: '#427AB3', borderRadius: moderateScale(15), borderWidth: moderateScale(1.5), padding: moderateScale(12), flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
                            <Text style={{fontSize: moderateScale(15), color: '#427AB3', fontWeight: 'bold' }} >Create New Playlist</Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                      <View>
                      <View style={{flexDirection: 'row', marginHorizontal: moderateScale(15), marginTop: moderateScale(30)}}>
                        <View style={{marginRight: moderateScale(10)}}>
                          {
                            myPlaylist.map((item, index) => {
                              if(index % 2 == 0){
                                return(
                                    <TouchableOpacity key={index} onPress={()=> addToOldPlaylist(item)}>
                                      <View style={styles.mainimage}>
                                      <CachedImage source={{uri: item.cover}} cacheKey={item.playlist_id} style={styles.image}/>
                                      </View>
...
const styles = StyleSheet.create({
  ...
  bottomViews: {
    ...
    alignItems: 'center',
    position: 'absolute', 
    top: height/4.2, borderTopRightRadius: moderateScale(25), borderTopLeftRadius: moderateScale(25)
  },
});
export default Home;
