...
import { ImageBackground, TextInput, Keyboard, TouchableWithoutFeedback, KeyboardAvoidingView, Modal,Image, View, Dimensions, StyleSheet, TouchableOpacity, Button, Text, ScrollView } from "react-native";
...
const Signup =({navigation}) =>{
  ...
  const loginform= async () => {
    if(email.length==0 && password.length==0 && last.length==0 && first.length==0){
      return updateError('All fields are required!', setError);
    }
    if(first.length==0){
      return updateError('First name is required!', setError1);
    }
    if(last.length==0){
      return updateError('Last name is required!', setError2);
    }
    if(email.length==0){
      return updateError('Email Address is required!', setError3);
    }
    if(!isValidEmail(email)){
      return updateError('Email Address is invalid!', setError3);
    }
    if(password.length==0){
      return updateError('Password is required!', setError4);
    }
    if(password.length<8){
      return updateError('Minimum of 8 characters!', setError4);
    }
    if(isChecked==false){
      return updateError('Please accept Terms and Condition!', setError5);
    }
    if(isValidEmail(email) && password.length>8 && last.length!=0 && first.length!=0){
      var name=first+" "+last;
      await fetch(global.link+`api/v1/register/email`, {
        ...
        body: JSON.stringify({email: email, first_name: first, last_name: last,
          password: password, subscription_id: 1, name: name
        })
        ...
        <ImageBackground source={require('../../assets/Login/login.png')} style={styles.container}>
          <Modal
            ...
            <View style={{height: Dimensions.get('window').height, width: Dimensions.get('window').width, flex: 1, backgroundColor: 'white', marginTop: '-100%'}}>
              <TouchableOpacity onPress={()=> [setModal1(false), setChecked(false)]} style={{ marginTop: -moderateScale(15), backgroundColor: 'rgba(4,157,217,0.8)', width: scale(25), height: scale(25), borderColor: 'white', borderWidth: 2, alignItems: 'center', justifyContent: 'center', borderRadius: scale(20), alignSelf: 'flex-end', marginRight: 20, shadowColor: 'black', shadowOffset: {width: 0, height:0}, shadowRadius: 3, shadowOpacity: 1}}>
                <Image source={{uri: 'https://cdn-icons-png.flaticon.com/512/1828/1828945.png'}} style={{width:scale(25), height:scale(25), tintColor: 'white',}}/>
              </TouchableOpacity>
              <View style={{height: '80%', width: Dimensions.get('window').width-moderateScale(50), marginLeft: moderateScale(30), alignItems: 'center', justifyContent: 'center'}}>
                <Text style={{fontSize: moderateScale(22), fontWeight: 'bold', color: 'black', textAlign: 'left', alignSelf: 'flex-start', marginTop: 15, marginBottom: 15}}>Terms and Conditions</Text>
                <ScrollView style={{marginBottom: scale(10)}}>
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>1. GENERAL INFORMATION</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginTop: 15}}>2. MEMBERSHIPS AND SUBSCRIPTIONS</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>3. CANCELLATION OF SERVICES</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 10}}>4. PROHIBITED USE OF THE PRODUCTS</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 10}}>5. MATERIALS OFFERED THROUGH THE PRODUCTS</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 10}}>6. AVAILABILITY OF PRODUCTS</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 10}}>7. USER MATERIAL</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 10}}>8. LINKS TO WEBSITES/HOME PAGE</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>9. PRODUCTS DISCLAIMER</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>10. MEDICAL DISCLAIMER</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>11. END USER LICENSE</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>12. DIGITAL MILLENIUM COPYRIGHT ACT (“DMCA”) NOTICE</Text>
                  ...
                  <Text style={{fontSize: moderateScale(16), marginRight: 15, fontWeight: 'bold', marginBottom: 5}}>13. GENERAL TERMS AND CONDITIONS</Text>
              ...
              <TouchableOpacity onPress={()=>[setModal1(false), setChecked(true)]} style={[styles.sub]}>
                <Text style={{fontSize: moderateScale(15), fontWeight: 'bold', color: '#fff'}}>Accept</Text>
              </TouchableOpacity>
        ...
        {error ? 
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={styles.inner}>
              <Image source={require('../../assets/Login/signup.png')} style={styles.bg}/>
              <Text style={styles.title}>Hello There! </Text>
              <Text style={styles.subtitle}>New here? Sign up now!</Text>
                <View style={styles.view}>
                  {error1 ? (<Text style={styles.error}>{error1}</Text>) : null}
                  <TextInput value={first} onChangeText={val=> setFirst(val)} style={styles.textinput} placeholder="First Name"/>
                </View>
                <View style={styles.view}>
                  {error2 ? (<Text style={styles.error}>{error2}</Text>) : null}
                  <TextInput value={last} onChangeText={val=> setLast(val)} style={styles.textinput} placeholder="Last Name"/>
                </View>
                <View style={styles.view}>
                ...
                <View style={styles.toggle}>
                  <Toggle 
                    ...
                    onTrue={() => [setChecked(true), setModal1(true)]} onFalse={() => setChecked(false)}
                  />
                  <TouchableOpacity>
                    <Text style={styles.toggletext}>I have read and accept Terms and Conditions</Text>
                  </TouchableOpacity>
                </View>
              {error5 ? (<Text style={styles.errortoggle}>{error5}</Text>) : null}
              <TouchableOpacity onPress={()=> loginform()} style={styles.button}>
                <Text style={styles.email}>Signup</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={()=> navigation.navigate("Login")} style={[styles.touch1]} >
...
const styles = StyleSheet.create({
  ...
  errortoggle: {
    textAlign: 'left', fontSize: moderateScale(11), color: '#FF6A6A', fontWeight: '500', marginTop: scale(5), marginBottom: scale(-10)
  },
  toggletext: {
    fontSize: moderateScale(13), fontWeight: '600', marginLeft: 10, textAlign: 'center'
  },
  sub: {
    height: verticalScale(40), backgroundColor: '#4C89C6', borderRadius: 10, justifyContent: 'center',  alignItems: 'center', marginHorizontal: moderateScale(30), shadowColor: 'black', shadowOffset: {width:0, height: 0}, shadowOpacity: 0.8
  },
  text2: {
    fontSize: moderateScale(11), textAlign: 'justify', lineHeight: moderateScale(25), marginRight: 15, 
  },
  text1: {
    fontSize: moderateScale(13), marginRight: 15, fontWeight: 'bold', marginTop: 20
  }
